...
import numpy as np
import pandas as pd
from models import Variety, ForecastData
...
def run_prophet_or_fallback_forecast(variety: Variety, metric_type: str = "yield_index", days: int = 30) -> ForecastResponse:
    ...
    # Try Prophet execution
    try:
        from prophet import Prophet

        # Build synthetic historical time series baseline for variety
        history_days = 90
        dates = [today - datetime.timedelta(days=i) for i in range(history_days, 0, -1)]

        base_val = variety.yield_potential_ton_ha if metric_type == "yield_index" else 25.0

        # Create realistic seasonal curve
        df_list = []
        ...
        # Robust mathematical fallback forecaster (trend + harmonic seasonality + uncertainty bounds)
        points = []
        base_val = variety.yield_potential_ton_ha if metric_type == "yield_index" else 30.0

        for day_offset in range(1, days + 1):
            future_date = today + datetime.timedelta(days=day_offset)
            date_str = future_date.strftime("%Y-%m-%d")

            # Seasonality formula simulating crop growth progression
